...
def main():
    ...
    st.title("Customer Query Assistant with RAG")
    st.markdown("---")
    ...
    if submit_button and user_query:
        if model_type == "Question Answering" and not context_text and not use_rag:
            st.error("Please provide context text for Question Answering mode (or enable RAG)")
        else:
                ...
                try:
                    ...
                    if use_rag:
                        with st.spinner("Retrieving relevant documents..."):
                            retrieved_docs = retrieve_documents(user_query, k=retrieval_k)
                            retrieved_context = "\n\n".join(retrieved_docs)
    ...
    st.markdown(
        ...
        Customer Query Assistant with RAG | Powered by Hugging Face Transformers & Streamlit
        </div>
        """,
